...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create concurrently-7368
  fly apps create concurrently-7368-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app concurrently-7368
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app concurrently-7368-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'concurrently-7368-staging' does not match app name in config file 'concurrently-7368'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name concurrently-7368-db
  fly postgres attach --postgres-app concurrently-7368-db --app concurrently-7368

  fly postgres create --name concurrently-7368-staging-db
  fly postgres attach --postgres-app concurrently-7368-staging-db --app concurrently-7368-staging
  ```
